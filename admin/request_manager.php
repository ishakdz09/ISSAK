...
<head>
    ...
    <title>Dooo - Request Manager</title>
...
<script>
    ...
    $('#datatable').dataTable({
        ...
        "processing": true,
        "serverSide": true,
        "ajax": "/admin/dashboard_api/request/get_all_request.php",
        "columns": [{
                "data": "1",
                render: function () {
                    return c_id++;
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    if (data == 0) {
                        ...
                        return '<div class="btn-group mr-1 mt-2"> <button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true"  aria-expanded="false">Accepted</button> <div class="dropdown-menu" style=""> <a class="dropdown-item" onclick="UpdateStatus(0,' +
                        row["2"] + ')" href="#">Pending</a> <a class="dropdown-item" onclick="UpdateStatus(1,' +
                        row["2"] + ')" href="#">Accepted</a> <a class="dropdown-item" onclick="UpdateStatus(2,' +
                        row["2"] + ')" href="#">Rejected</a> <a class="dropdown-item" onclick="DeleteStatus(2,' +
                        row["2"] + ')" href="#">Delete</a> </div>';
                    }else if (data == 2) {
                        return '<div class="btn-group mr-1 mt-2"> <button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true"  aria-expanded="false">Rejected</button> <div class="dropdown-menu" style=""> <a class="dropdown-item" onclick="UpdateStatus(0,' +
                        row["2"] + ')" href="#">Pending</a> <a class="dropdown-item" onclick="UpdateStatus(1,' +
                        row["2"] + ')" href="#">Accepted</a> <a class="dropdown-item" onclick="UpdateStatus(2,' +
                        row["2"] + ')" href="#">Rejected</a> <a class="dropdown-item" onclick="DeleteStatus(2,' +
                        row["2"] + ')" href="#">Delete</a> </div>';
    ...
    function UpdateStatus(status, request_id) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "dashboard_api/update_request_status.php",
    ...
    function DeleteStatus(status, request_id) {
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "dashboard_api/delete_request.php",
                    contentType: 'application/json',
                    data: JSON.stringify(jsonObjects),
                    dataType: 'text',
                    success: function (response) {
